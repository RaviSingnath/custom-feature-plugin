...
foreach ($my_cpts_post as $post_type) {
    $filters[] = create_filter_callback($post_type);
}

// Register the filters
foreach ($filters as $filter) {
    add_filter('graphql_input_fields', $filter, 20, 3);
}

function create_filter_callback($post_type) {
    return function ($fields, $type_name, $config) use ($post_type) {
        $my_post_type = ucfirst($post_type);
        $my_type_name = "Create" . $my_post_type . "Input";
